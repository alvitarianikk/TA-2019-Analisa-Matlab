function error_count4=subblok4berreyleigh(PAPD,BD_data,symbol_per_carrier,subcarrier,Phase_Set,baseband_datalength,baseband_data,GI,EbNodB,error_count);
% ------------ KANAL reyleigh ----------- %
...
for k=0:(symbol_per_carrier-1) 
        for i=1:subcarrier  %sampai 512 kali (data subcarriernya)
             xx(1,i+GI+(subcarrier+GI)*k)=PAPD(1,i+subcarrier*k); % misal 1 data nilainya = 0.019531250000000 - 0.019531250000000i
        end 
        for i=1:GI  %i=(1 sampai 128) GI= 1/4*N=128
            xx(1,i+(GI+subcarrier)*k)=PAPD(1,i+subcarrier-GI+subcarrier*k); %imaginer seri
        end
